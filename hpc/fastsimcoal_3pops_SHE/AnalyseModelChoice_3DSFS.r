...
# - obsfilename_1snp : file name of multidimensional observed SFS (3D-MSFS) with 1 random SNP per RAD locus supposed to be in folder ./ObsSFS

# Load functions
source("utilFunctions3DSFS.r")
...
pop <- "" 
model <- "SHE_WSRS"
pop.names <- c("SHE","WS","RS")
obsfilename <- "SHE_WS_RS_DSFS.obs" # supposed to be in folder ./ObsSFS
...
setwd("./FilesRequired")
  ...
  pdf(file=paste("FitJoint3D_SFS_orderentries_", model ,".pdf",sep=""), width=12, height=6)
  ...
  lines(log10(exp.sfs[eval[indexorder]]),col=3)
  ...
  par(mfrow=c(3,1), mar=c(10,5,2,1), las=2)
  ...
  # save the likelihood of 100 reps with allsnps
