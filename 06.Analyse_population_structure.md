...
get_f("tur.rep2.5") #rep7 has the least amount of residuals
...
#read in GDM env data (long, lat, water depth info of each sample) in .hpc/structure in this repo
...
#from thin1000_maf005_obshet05.vcf, keep Scotia Sea samples only
#genotype file genearted from from -012 output of vcftools
#Combine out.012 , out.012.idv , out.012.pos into a csv file & replace -1 with NA
...
env <- read.csv("./RDA_scotia_tur_env_var.csv") #in ./hpc/structure in this repo
...
screeplot(tur_scotia.rda) #first RDA explain most of the variance
...
#Plot the RDA 
#Red = snps, indv = black circles, vlue vectors = env predictors
