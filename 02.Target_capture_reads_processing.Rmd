---
title: "Target capture data: Reads processing, quality control and variant calling"
...
```
Discard reads with phred quality (Q) less than 20, trim polyG (problem with early Novaseq):
```{bash, error=TRUE, results='hide'}
...
```
Map cleaned reads to consensus loci (fasta file used to generate target capture baits):
```{bash, error=TRUE, results='hide'}
...
Call SNPs across samples using bcftools (inlcuding both monomorphic and polymorphic sites):
```{bash, error=TRUE, results='hide'}
REF=./turqueti/tarcap/reference/unique_consensus_loci.fa
